...
from .secrets_helper import get_gcp_project
            ...
            if not credentials:
                ...
                for key_path in local_key_paths:
                    if key_path.exists():
                        try:
                            logger.info(
                                f"Loading SQL credentials from local key: {key_path}"
                            )
                            credentials = (
                            ...
                            logger.debug(
                                f"Failed to load SQL credentials from {key_path}: {e}"
                            )
    ...
    def _initialize_connection(self) -> None:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                logger.info(
                    f"✅ Connected to Cloud SQL: {self.instance_connection_name}"
                )
...
def get_cloud_sql_instance() -> CloudSQLHelper:
    ...
    if _cloud_sql_instance is None:
        ...
        instance_connection_name = config["instance"]
        ...
        if not instance_connection_name or not password:
            raise ValueError(
                "CLOUDSQL_INSTANCE and CLOUDSQL_PASSWORD must be set in .streamlit/secrets.toml or .env"
            )
