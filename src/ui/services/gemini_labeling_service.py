...
class GeminiLabelingService:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-1.5-flash"):
        ...
        if HAS_VERTEX_AI:
            try:
                # Initialize Vertex AI (uses Application Default Credentials or google_api_key)
                ...
                if project_id:
                    ...
                    logger.info(f"✅ Initialized Gemini via Vertex AI ({project_id}) using {model}")
                    return
            except Exception as e:
        ...
        try:
            ...
            self.model = genai.GenerativeModel(model)
            self.model_name = model
            logger.info(f"✅ Initialized Gemini with Application Default Credentials using {model}")
            return
        except Exception as e:
    ...
    def generate_label(
        ...
        prompt = f"""Analyze this neuron's activation pattern and generate a descriptive label.
...
{{
        ...
        for attempt in range(max_retries):
            try:
                ...
                if start >= 0 and end > start:
                    ...
                    logger.warning(f"Could not parse JSON response for neuron {neuron_id}")
    ...
    def label_neurons_with_verification(
        ...
        logger.info(f"🧠 Preparing {len(neurons_data)} neurons for verification...\n")
        ...
        candidates = [
            n for n in scored_neurons 
            if n['quality_score'] >= confidence_threshold
        ][:max_labels]
        
        logger.info(f"📊 Found {len(candidates)} candidate neurons for labeling\n")
    ...
    def _score_neurons(self, neurons_data: Dict) -> List[Dict]:
        ...
        for neuron_id, neuron_info in neurons_data.items():
            ...
            # Sparsity bonus
            score += min(len([i for i in top_items if i.get('activation', 0) > 0.1]), 10)
    ...
    def _show_verification_ui(self, candidates: List[Dict]) -> Dict[str, Dict]:
        ...
        for i, neuron in enumerate(candidates[:5], 1):  # Show first 5
            ...
            logger.info(f"{i}. Neuron {neuron['neuron_id']} (Quality: {neuron['quality_score']:.1%})")
        ...
        logger.info("  Option 1: Approve all candidates → run: labeler.verify_and_label_all()")
        logger.info("  Option 2: Select specific neurons → run: labeler.label_specific([0, 3, 5])")
    ...
    def verify_and_label_all(self, candidates: List[Dict]) -> Dict[str, Dict]:
        ...
        for i, neuron in enumerate(candidates, 1):
            try:
                logger.info(f"[{i}/{len(candidates)}] Labeling neuron {neuron['neuron_id']}...")
    ...
    def label_specific(self, candidate_indices: List[int]) -> Dict[str, Dict]:
        ...
        logger.info(f"✅ Proceeding to label {len(candidate_indices)} selected neurons...\n")
