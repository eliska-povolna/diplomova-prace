    ...
    with tab_speed:
        ...
        if 'latency' in results:
            ...
            with col1:
                st.metric(
                    "Mean Latency",
                    f"{latency.get('mean', 0):.3f}s"
                )
            with col2:
                st.metric(
                    "P95 Latency",
                    f"{latency.get('p95', 0):.3f}s"
                )
            with col3:
                st.metric(
                    "Max Latency",
                    f"{latency.get('max', 0):.3f}s"
                )
        else:
...
def show_placeholder_results():
    ...
    with tab_speed:
        ...
        fig.add_vline(0.847, line_dash="dash", line_color="green", 
                      annotation_text="Mean", annotation_position="top right")
        fig.add_vline(1.234, line_dash="dash", line_color="orange",
