...
def main():
    ...
    dfb = con.execute(
        f"SELECT business_id, name, categories, attributes FROM read_parquet('{business_glob}')"
    ).fetchdf()
    ...
    item_df = pd.DataFrame(
        {"business_id": list(item2idx.keys()), "i": list(item2idx.values())}
    )
    ...
    for h in range(H):
        scores = W[h]  # (I,)
        top_idx = np.argpartition(scores, -args.top_items_per_neuron)[
            -args.top_items_per_neuron :
        ]
        mean_tfidf = X[top_idx].mean(axis=0).A1
        top_tag_idx = np.argsort(-mean_tfidf)[: args.top_tags_per_neuron]
        tags = [
            (vocab[j], float(mean_tfidf[j])) for j in top_tag_idx if mean_tfidf[j] > 0
        ]
