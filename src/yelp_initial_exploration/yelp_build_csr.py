...
def main():
    ap = argparse.ArgumentParser(
        description="Build CSR and ID maps from Yelp Parquet + DuckDB"
    )
    ap.add_argument(
        "--parquet_dir",
        type=Path,
        required=True,
        help="Dir with parquet (business/, review/)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=4.0,
        help="stars >= threshold => implicit 1",
    )
    ap.add_argument(
        "--any_review",
        action="store_true",
        help="If set, any review counts as implicit 1",
    )
    ap.add_argument(
        "--state_filter", type=str, default=None, help="Optional: only this state code"
    )
    ...
    if args.year_min or args.year_max:
        ...
        if args.year_min:
            year_cond.append(
                f"CAST(strftime(date, '%Y') AS INTEGER) >= {int(args.year_min)}"
            )
        if args.year_max:
            year_cond.append(
                f"CAST(strftime(date, '%Y') AS INTEGER) <= {int(args.year_max)}"
            )
    ...
    df[["user_id", "business_id", "ts", "implicit", "u", "i"]].to_parquet(
        out_interactions, compression="zstd", index=False
    )
    ...
    R = coo_matrix(
        (vals, (df["u"].values, df["i"].values)), shape=(n_users, n_items)
    ).tocsr()
    ...
    print(
        "Saved:",
        out_interactions,
        args.out_dir / "processed_train.npz",
        args.out_dir / "user2index.pkl",
        args.out_dir / "item2index.pkl",
    )
