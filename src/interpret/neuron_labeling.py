    ...
    def label_neurons(
        ...
        for neuron_idx, profile in neuron_profiles.items():
            ...
            for item_id, activation in max_items[:10]:  # Top 10
                # item_id might be an index into business_metadata list or a business_id string
                meta = business_metadata.get(item_id, {})
                
                # If no metadata found and item_id is an index, try looking it up by position
