...
\begin{description}
    \item \textbf{AI} (Artificial Intelligence): umělá inteligence
    \item \textbf{API} (Application Programming Interface): rozhraní pro programování aplikací
    \item \textbf{CBF} (Content-Based Filtering): obsahové filtrování
    \item \textbf{CF} (Collaborative Filtering): kolaborativní filtrování
    \item \textbf{CFAE} (Collaborative Filtering Autoencoder): autoenkodér pro kolaborativní filtrování
    \item \textbf{GPU} (Graphics Processing Unit): grafická procesorová jednotka
    \item \textbf{HCI} (Human-Computer Interaction): interakce člověka s počítačem
    \item \textbf{JSON} (JavaScript Object Notation): textový formát pro výměnu strukturovaných dat
    \item \textbf{LLM} (Large Language Model): velký jazykový model
    \item \textbf{NDCG} (Normalized Discounted Cumulative Gain): normalizovaný diskontovaný kumulativní zisk
    \item \textbf{POI} (Point of Interest): bod zájmu (např. restaurace, památka, apod.)
    \item \textbf{RS} (Recommender System): doporučovací systém
    \item \textbf{SAE} (Sparse Autoencoder): řídký autoenkodér
    \item \textbf{TF-IDF} (Term Frequency -- Inverse Document Frequency): frekvence slova vůči inverzní frekvenci dokumentu
    \item \textbf{UI} (User Interface): uživatelské rozhraní
    \item \textbf{VAE} (Variational Autoencoder): variační autoenkodér
    \item \textbf{XAI} (Explainable Artificial Intelligence): vysvětlitelná umělá inteligence
\end{description}
